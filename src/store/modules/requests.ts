...
import { getArticles, getArticlesByTitle, getArticlesBySort } from "@/services/modules/articles";
...
export default {
	...
	actions: {
		...
		async getArticlesBySort({ state, commit }: ActionsParam) {
			return await getArticlesBySort(state._start, state._sort).then((res: Article[]) => {
				commit("setArticles", res);
				commit("setStart");
				return res;
			});
